...
export default class Bitstring {
  // Lazy getters avoid touching ERTS/TextEncoder/TextDecoder while the module
  // graph is still initializing in bundled test/runtime environments.
  static #decoder = null;
  static get #textDecoder() {
    if (!$.#decoder) $.#decoder = ERTS.utf8Decoder;
    return $.#decoder;
  }

  static #encoder = null;
  static get #textEncoder() {
    if (!$.#encoder) $.#encoder = new TextEncoder("utf-8");
    return $.#encoder;
  }
  ...
  static fromSegmentWithStringValue(segment) {
    ...
    const sourceBytes = $.#textEncoder.encode(valueStr);
  ...
  static maybeSetBytesFromText(bitstring) {
    if (bitstring.bytes === null) {
      bitstring.bytes = $.#textEncoder.encode(bitstring.text);
    }
  }
  ...
  static maybeSetTextFromBytes(bitstring) {
    if (bitstring.text === null) {
      try {
        bitstring.text = $.#textDecoder.decode(bitstring.bytes);
      } catch {
