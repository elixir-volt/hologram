...
const Erlang_Math = {
  ...
  "exp/1": (number) => {
    ...
    // Some libm implementations return Math.exp(1) one ULP above Math.E.
    // Using the language constant keeps this identity deterministic across
    // JavaScript engines and matches Erlang's result.
    const result = value === 1 ? Math.E : Math.exp(value);
